# CA2 Progress Tracker

This file tracks CA2 rubric progress with checklist boxes.

Legend:
- `Done`: requirement is implemented and verified in code.
- `Partial`: implemented in part, but still missing rubric expectations.
- `To do`: not implemented yet.

How to use:
- Put `✗` only when a requirement fully matches the rubric.
- Leave the cell empty for `Partial` and `To do` items.
- Update status as you complete work.

Last reviewed: 2026-02-26

| Done | Status | Requirement | Max |
|---|---|---|---:|
| | Partial | Provide the user with an excellent visual appearance | 5 |
| | Partial | Provide the user with an excellent user experience | 5 |
| ✗ | Done | Include any number of product images for each product; images display appropriately on mobile/tablet/desktop | 2.5 |
| ✗ | Done | Include a shopping cart | 2.5 |
| | To do | Include payments | 2.5 |
| ✗ | Done | Hamburger menu on mobile and horizontal menu on tablet/computer | 2.5 |
| ✗ | Done | Use cards on mobile and tables on tablet/computer | 2.5 |
| | Partial | Show less data on a tablet row compared to a computer | 2.5 |
| ✗ | Done | Clicking a row/card opens modal with all data | 2.5 |
| ✗ | Done | Use tokens and a `.PEM` file | 2.5 |
| | Partial | User-friendly client-side validation | 5 |
| ✗ | Done | Router validation | 2.5 |
| ✗ | Done | MongoDB model validation | 2.5 |
| ✗ | Done | Implement middleware | 2.5 |
| ✗ | Done | Implement middleware error handling | 2.5 |
| ✗ | Done | Store data in MongoDB (localhost) | 2.5 |
| ✗ | Done | Encrypt administrator and customer passwords | 2.5 |
| | To do | Filter (with search and sort) | 2.5 |
| | Partial | Search (with filter and sort) | 2.5 |
| | To do | Sort (with filter and search) | 2.5 |
| | Partial | Admin create product, including error handling UI | 2.5 |
| | Partial | Admin edit product, including error handling UI | 2.5 |
| ✗ | Done | Admin can add multiple images when adding product; images display | 2.5 |
| | To do | Admin adjust stock level, including error handling UI | 2.5 |
| | To do | Admin view customers | 2.5 |
| | To do | Admin view customers purchase history | 2.5 |
| ✗ | Done | Admin delete product | 2.5 |
| | To do | Filter/search/sort for products, customers, and purchase history | 2.5 |
| | Partial | Add new record (including images), with error handling UI | 2.5 |
| | Partial | Edit records (including images), with error handling UI | 2.5 |
| | Partial | Delete products, including modal | 2.5 |
| | Partial | Logged-in customer purchase products | 2.5 |
| | To do | Logged-in customer view purchase history (with filter/search/sort/images) | 2.5 |
| | To do | Logged-in customer return products | 2.5 |
| | To do | Logged-in customer create profile with photo, including error handling UI | 2.5 |
| | To do | Logged-in customer edit profile with photo, including error handling UI | 2.5 |
| | To do | Guest customer purchase products and save customer/product details for sales history, including error handling UI | 2.5 |
